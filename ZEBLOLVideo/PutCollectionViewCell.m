...
@implementation PutCollectionViewCell {
...
- (void)tapPlayVideo:(UITapGestureRecognizer *)recognizer {
    if (_putDelegate && [_putDelegate respondsToSelector:@selector(putPlayVideo:flv:title:)]) {
        [_putDelegate putPlayVideo:self flv:_videoModel.flv title:_videoModel.title];
    }
}
...
- (void)reloadCell {
    [_mainImageView sd_setImageWithURL:[NSURL URLWithString:_videoModel.big] placeholderImage:[UIImage imageNamed:@"loading_3"] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
        
    }];
    _userImageView.image = [UIImage imageNamed:@"user-0"];
    _lookLable.text = _videoModel.click;
    _titleLable.text = _videoModel.title;
    _userNameLable.text = _videoModel.nickname;
}
@end
